...
public class SearchResult {
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            for (int i = 0; i < expectedTableHeaders.size(); i++) {
                String expTHead = expectedTableHeaders.get(i);

                int column = i + 1;
                WebElement headerElement =
                        driver.findElement(By.xpath("//table/thead/tr/th[" + column + "]"));
                String actualTHeader = headerElement.getText();
                if (actualTHeader.equals(expTHead)) {
                    status = true;
                }
            }
            ...
            for (int i = 0; i < expectedTableBody.size(); i++) {
                ...
                for (int j = 0; j < completeRow.size(); j++) {
                    String excptTBody = completeRow.get(j);
                    int row = i + 1;
                    int column = j + 1;
                    WebElement tableBodyElement = driver
                            .findElement(By.xpath("//table/tbody/tr[" + row + "]/td[" + column + "]"));
                    String actualTBody = tableBodyElement.getText();
                    if (!actualTBody.equals(excptTBody)) {
                        status = false;
                    }
                }
            }
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
            ...
            WebElement sizeDropdown = driver.findElement(By.xpath("//button[text()='Size chart']"));
